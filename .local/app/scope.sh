#!/usr/bin/env bash
# From https://github.com/ranger/ranger/blob/391f061cb0b0cfa8266c0651f2a6d948f22e01dd/ranger/data/scope.sh
...
handle_extension() {
    case "${FILE_EXTENSION_LOWER}" in
        ...
        rpm|rz|t7z|tar|tbz|tbz2|tgz|tlz|txz|tZ|tzo|war|xpi|xz|Z|zip)
            ouch list -t -- "${FILE_PATH}" && exit 5  # ilyagr
            atool --list -- "${FILE_PATH}" && exit 5
            bsdtar --list --file "${FILE_PATH}" && exit 5
            lsar -- "${FILE_PATH}" && exit 5  # ilyagr
            exit 1;;
        rar)
